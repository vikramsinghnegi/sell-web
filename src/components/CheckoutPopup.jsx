...
import { FaUser } from "react-icons/fa";
import { loadRazorpayScript } from "../utils/razorpay";
import api from "../utils/api"; // helper file jisme createOrder aur verifyPayment hai
...
const CheckoutPopup = ({ plan, close }) => {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // 1️⃣ Create order via helper function
      let orderData;
      try {
        orderData = await api.createOrder(Number(plan.price));
      } catch (err) {
        console.error("createOrder failed:", err);
        alert(
          "Order creation failed. Check console for details. Possible CORS / backend issue."
        );
        setLoading(false);
        return;
      }

      if (!orderData || !orderData.success || !orderData.order?.id) {
        console.error("Invalid order response:", orderData);
        alert("Order creation failed — backend returned invalid response.");
        setLoading(false);
        return;
      }
